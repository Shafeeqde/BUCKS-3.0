...
const SearchSuggestions: React.FC<SearchSuggestionsProps> = ({ suggestions, onSuggestionClick, title }) => {
  ...
  return (
    <div className="mt-2 bg-card rounded-lg shadow-lg p-4 max-h-60 overflow-y-auto custom-scrollbar">
      ...
      <div className="flex flex-wrap gap-2">
        {suggestions.map((suggestion, index) => (
          ...
          >
            <ChevronRightIcon className="w-4 h-4 mr-1 text-muted-foreground" />
            {suggestion}
          </Button>
        ))}
      </div>
