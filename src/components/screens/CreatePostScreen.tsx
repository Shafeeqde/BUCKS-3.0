...
const CreatePostScreen: React.FC<CreatePostScreenProps> = ({ onPost, onCancel }) => {
  ...
  // Future: const [selectedImage, setSelectedImage] = useState<File | null>(null);
  // Future: const [imageUrlPreview, setImageUrlPreview] = useState<string | null>(null);
  ...
  const handlePostSubmit = async () => {
    ...
    // For now, we'll pass undefined for imageUrl as image upload isn't fully implemented
    onPost(postContent); 
    
    setIsPosting(false);
    setPostContent(''); 
    // Navigation back is handled by onPost in page.tsx
  };
    ...
    <div className="flex flex-col h-full p-4 bg-muted/30">
      ...
      <Card className="flex-grow flex flex-col shadow-xl">
        ...
        <CardContent className="flex-grow flex flex-col space-y-4">
          ...
          {imageUrlPreview && (
            <div className="mt-2">
              <img src={imageUrlPreview} alt="Preview" className="max-h-40 rounded-md border" />
            </div>
          )}
          */}
        </CardContent>
        <CardFooter className="border-t pt-4 flex flex-col sm:flex-row justify-between items-center gap-3">
          <div className="flex items-center space-x-2">
            <Button variant="outline" size="sm" onClick={() => toast({ title: "Add Media", description: "Media upload coming in Phase 2!"})} disabled={isPosting}>
              <PhotoIcon className="mr-2 h-4 w-4" /> Add Media
            </Button>
            {/* Add more action buttons here like Tag Friends, Add Location etc. */}
          </div>
          <Button 
