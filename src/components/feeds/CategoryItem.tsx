...
const CategoryItem: React.FC<CategoryItemProps> = ({ category, onClick }) => {
  ...
  return (
    <div
      className="flex-shrink-0 flex flex-col items-center cursor-pointer w-20"
    ...
    >
      {category.type === 'moments' ? (
        <div className={cn(`w-14 h-14 rounded-full flex items-center justify-center text-primary border-2 ${borderColor}`, category.color || 'bg-primary/10')}>
          <Plus className="w-8 h-8" />
        </div>
      ) : category.image ? (
        <div className={`w-14 h-14 rounded-full border-2 ${borderColor} overflow-hidden flex items-center justify-center`}>
        ...
        <div className={`w-14 h-14 rounded-full flex items-center justify-center text-primary-foreground border-2 ${borderColor} ${category.color || 'bg-primary'}`}>
